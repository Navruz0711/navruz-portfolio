...
export const FloatingDock = ({
  ...
  return (
    <>
      ...
      <FloatingDockMobileInline items={items} className={cn("flex md:hidden", mobileClassName)} />
    </>
  );
};

/** Simple inline row of icons for mobile — no magnification, just a clean grid */
const FloatingDockMobileInline = ({
  ...
  return (
    <div className={cn("flex flex-wrap gap-2 pointer-events-auto", className)}>
      {items.map((item) => (
        <div
          key={item.title}
          className="h-9 w-9 rounded-full bg-secondary/40 backdrop-blur-md flex items-center justify-center border border-border/50"
          title={item.title}
        >
